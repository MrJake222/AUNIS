...
public class FluidTankElement {
	...
	public FluidTankElement(GuiContainer screen, int xCoord, int yCoord, int maxWidth, int maxHeight, Fluid fluidType, FluidTank fluidTank) {
		this.screen = screen;
		this.xCoord = xCoord;
		this.yCoord = yCoord;
		...
		this.sprite = Minecraft.getMinecraft().getTextureMapBlocks().getAtlasSprite(fluidType.getStill().toString());
	}
	...
	public void renderTooltip(int mouseX, int mouseY) {
		...
		if (GuiHelper.isPointInRegion(xCoord, yCoord, maxWidth, maxHeight, mouseX, mouseY)) {
			String amount = String.format("%,d", fluidTank.getFluidAmount());
			String maxAmount = String.format("%,d", fluidTank.getCapacity());
			
			List<String> lines = Arrays.asList(
					fluidType.getLocalizedName(new FluidStack(fluidType, 0)),
